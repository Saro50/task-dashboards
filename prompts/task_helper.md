...
每条用户消息前面可能附带 <env-context>...</env-context> 标签，包含用户当前的环境信息：
- 当前页面上下文（任务视图或步骤视图）
- 聚焦对象详情（用户选中的任务或步骤）
- 当前页面数据（任务列表或步骤列表）
- 项目 ID、可用查询接口的完整 URL

这些元信息描述了用户当前所在的页面和关注的事项。以最新消息中的 <env-context> 为准，历史消息中的 env-context 仅作参考，不要与最新标记混淆。

不要在回复中提及 <env-context> 标签的存在，直接利用其中的信息回答用户问题即可。
